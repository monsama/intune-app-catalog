function Global:Show-GettingStartedGuideDialog {
    ...
    $blocks = @(
        ...
        @{ Type = "Body"; Text = "1. Press `"Checks...`" (Intune group on the toolbar) - or `"Get apps from Intune...`" on an empty catalog, which starts the run for you.`n2. Every app in Intune the catalog doesn't have yet shows up as `"In Intune, not in the catalog`" - `"Show: Intune link`" narrows the list to those.`n3. Select the ones you want and press `"Add to catalog...`":`n     - one app opens in the App Editor first, so you can set its winget ID and Required/Available/Uninstall groups right away;`n     - several are read from Intune and added in one go, with their metadata and groups.`n4. Anything added can be changed later - open it in the App Editor whenever you're ready." }
        ...
        @{ Type = "Body"; Text = "- `"Checks...`" (above) is also the tool for ongoing drift, not just first-time import - it lists apps renamed in Intune since, catalog apps whose App ID no longer exists in Intune, and for every deployed app each field, group, dependency and assignment that differs, each with its fix: Pull if Intune is right, Push if the catalog is.`n- `"Check Intune drift when this app starts`" (Settings > Automatic checks) runs the quick part of that check once every time the app opens, and only says something if it actually finds a difference - useful if more than one person works from this catalog.`n- `"Pull metadata and groups from Intune...`" (right-click an app) updates the LOCAL catalog to match what's live in Intune, for when Intune is the one with the current truth." }
    )
